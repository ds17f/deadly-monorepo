#!/usr/bin/env bash
#
# Push the staged LE certs from .secrets/le-cert/ into the Hetzner Caddy
# data volume. Phase 1.3 of the migration. Run AFTER provisioning the HZ
# box (1.2). Safe to re-run any time — busybox tar overwrites in place.
#
# Operates directly on the named volume (deadly_caddy_data) via a
# throwaway busybox container, so Caddy doesn't need to be stopped.
#
# Usage:
#   scripts/push-cert-to-hetzner.sh <HZ_IP> [hostname ...]
#   HZ_IP=1.2.3.4 scripts/push-cert-to-hetzner.sh [hostname ...]
#
# Default hostnames: thedeadly.app, share.thedeadly.app, www.thedeadly.app.
# Each must already be staged at .secrets/le-cert/<host>.tar.gz.
...
STAGE_DIR="$REPO_ROOT/.secrets/le-cert"
...
# Pre-flight: every staged tarball must exist and be unexpired.
for HOSTNAME in "${HOSTNAMES[@]}"; do
  TARBALL="$STAGE_DIR/$HOSTNAME.tar.gz"
  if [ ! -f "$TARBALL" ]; then
  ...
  if ! openssl x509 -in "$TMP_CRT" -noout -checkend 0 >/dev/null; then
    rm -f "$TMP_CRT"
    echo "error: staged cert for $HOSTNAME has expired. Re-run pull-cert-from-do.sh." >&2
    exit 1
  fi
  DAYS_LEFT=$(( ( $(date -d "$(openssl x509 -in "$TMP_CRT" -noout -enddate | cut -d= -f2)" +%s) - $(date +%s) ) / 86400 ))
  echo "    $HOSTNAME: valid for $DAYS_LEFT more days"
  if [ "$DAYS_LEFT" -lt 30 ]; then
    echo "warning: $HOSTNAME cert has <30 days left. Consider re-pulling from DO after forcing renewal there." >&2
  fi
  rm -f "$TMP_CRT"
done
...
echo
echo "==> Done. ${#HOSTNAMES[@]} cert(s) in place."
echo "    If Caddy is already running, restart it (a reload will not re-scan storage):"
echo "    ssh deploy@$HZ_IP 'cd /opt/deadly && docker compose restart caddy'"
