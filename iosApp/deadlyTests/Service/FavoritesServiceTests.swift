...
struct FavoritesServiceTests {
    ...
    let db: AppDatabase
    let service: FavoritesServiceImpl
    ...
    private func favoriteRecord(for showId: String) throws -> FavoriteShowRecord? {
        try db.read { db in try FavoriteShowRecord.fetchOne(db, key: showId) }
    }
    ...
    @Test("addToFavorites inserts FavoriteShowRecord and sets isFavorite on ShowRecord")
    func addToFavorites() throws {
        try insertShow(showId: "1977-05-08", date: "1977-05-08", year: 1977, month: 5)

        try service.addToFavorites(showId: "1977-05-08")

        let favoriteRec = try favoriteRecord(for: "1977-05-08")
        #expect(favoriteRec != nil)

        let showRec = try showRecord(for: "1977-05-08")
        #expect(showRec?.isFavorite == true)
        #expect(showRec?.favoritedAt != nil)
    }

    @Test("removeFromFavorites deletes FavoriteShowRecord and clears isFavorite on ShowRecord")
    func removeFromFavorites() throws {
        try insertShow(showId: "1977-05-08", date: "1977-05-08", year: 1977, month: 5)
        try service.addToFavorites(showId: "1977-05-08")

        try service.removeFromFavorites(showId: "1977-05-08")

        let favoriteRec = try favoriteRecord(for: "1977-05-08")
        #expect(favoriteRec == nil)

        let showRec = try showRecord(for: "1977-05-08")
        #expect(showRec?.isFavorite == false)
        #expect(showRec?.favoritedAt == nil)
    }

    @Test("isFavorite returns true after add, false after remove")
    func isFavoriteToggle() throws {
        try insertShow(showId: "1977-05-08", date: "1977-05-08", year: 1977, month: 5)

        #expect(try service.isFavorite(showId: "1977-05-08") == false)

        try service.addToFavorites(showId: "1977-05-08")
        #expect(try service.isFavorite(showId: "1977-05-08") == true)
    ...
    func refreshSortByDateAddedDescending() throws {
        ...
        try service.addToFavorites(showId: "1972-08-27")
        Thread.sleep(forTimeInterval: 0.01)
        try service.addToFavorites(showId: "1989-10-09")
    ...
    func refreshSortByVenue() throws {
        try insertFixtureShows()
        try service.addToFavorites(showId: "1977-05-08")   // Barton Hall
        try service.addToFavorites(showId: "1989-10-09")   // Hampton Coliseum
        try service.addToFavorites(showId: "1972-08-27")   // Veneta Fairgrounds
    ...
    func refreshSortByRatingDescending() throws {
        try insertFixtureShows()
        try service.addToFavorites(showId: "1977-05-08")   // 4.9
        try service.addToFavorites(showId: "1972-08-27")   // 4.5
        try service.addToFavorites(showId: "1989-10-09")   // 4.7
