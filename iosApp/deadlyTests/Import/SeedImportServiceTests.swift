...
struct SeedImportServiceTests {
    ...
    func roundTrip() throws {
        ...
        let importer = SeedImportService(database: db, showDAO: ShowDAO(database: db))
    ...
    @Test("Non-destructive: a non-favorites CASCADE child survives the seed refresh")
    func preservesNonFavoritesUserData() throws {
        let db = try AppDatabase.makeEmpty()
        let now = Int64(Date().timeIntervalSince1970 * 1000)

        // Pre-existing show + a user recording preference hanging off it via a
        // CASCADE FK. The old favorites-only snapshot did NOT preserve this table,
        // so `DELETE FROM shows` would have silently wiped it. ADR-0009 upserts
        // `shows` instead, so any child holding user data survives.
        try db.write { database in
            try database.execute(sql: """
                INSERT INTO shows (showId, date, year, month, yearMonth, band, venueName, createdAt, updatedAt)
                VALUES ('s1', '1977-05-08', 1977, 5, '1977-05', 'Grateful Dead', 'Old Venue', ?, ?)
            """, arguments: [now, now])
            try database.execute(sql: """
                INSERT INTO recording_preferences (showId, recordingId, updatedAt)
                VALUES ('s1', 'user-picked-rec', ?)
            """, arguments: [now])
        }

        let seedURL = try makeSeedFile()
        defer { try? FileManager.default.removeItem(at: seedURL) }
        _ = try SeedImportService(database: db, showDAO: ShowDAO(database: db)).importSeed(at: seedURL)

        let (prefCount, prefRec, venue): (Int?, String?, String?) = try db.read { database in
            (try Int.fetchOne(database, sql: "SELECT COUNT(*) FROM recording_preferences WHERE showId='s1'"),
             try String.fetchOne(database, sql: "SELECT recordingId FROM recording_preferences WHERE showId='s1'"),
             try String.fetchOne(database, sql: "SELECT venueName FROM shows WHERE showId='s1'"))
        }
        // The user's recording preference survived the catalog refresh…
        #expect(prefCount == 1)
        #expect(prefRec == "user-picked-rec")
        // …and the catalog row for s1 was updated in place, not deleted+reinserted.
        #expect(venue == "Barton Hall")
    }

    @Test("Reconcile derives isFavorite from live favorites only (tombstones excluded)")
    func reconcilesFavoriteFlag() throws {
        let db = try AppDatabase.makeEmpty()
        let favoritesDAO = FavoritesDAO(database: db)
        let now = Int64(Date().timeIntervalSince1970 * 1000)

        // Both shows must exist before favoriting (FK), and the denormalized flag
        // starts deliberately wrong (0) to prove the reconcile actively repairs it.
        try db.write { database in
            for id in ["s1", "s2"] {
                try database.execute(sql: """
                    INSERT INTO shows (showId, date, year, month, yearMonth, band, venueName, isFavorite, createdAt, updatedAt)
                    VALUES (?, '1977-05-08', 1977, 5, '1977-05', 'Grateful Dead', 'V', 0, ?, ?)
                """, arguments: [id, now, now])
            }
        }
        // s1 is live-favorited; s2 is favorited then tombstoned (deletedAt set).
        try favoritesDAO.add(FavoriteShowRecord(showId: "s1", addedToFavoritesAt: now, updatedAt: now))
        try favoritesDAO.add(FavoriteShowRecord(showId: "s2", addedToFavoritesAt: now, updatedAt: now))
        try db.write { database in
            try database.execute(sql: "UPDATE favorite_shows SET deletedAt = ? WHERE showId = 's2'", arguments: [now])
        }
        ...
        let (s1Fav, s2Fav, favCount): (Int?, Int?, Int?) = try db.read { database in
            (try Int.fetchOne(database, sql: "SELECT isFavorite FROM shows WHERE showId='s1'"),
             try Int.fetchOne(database, sql: "SELECT isFavorite FROM shows WHERE showId='s2'"),
             try Int.fetchOne(database, sql: "SELECT COUNT(*) FROM shows WHERE isFavorite=1"))
        }
        #expect(s1Fav == 1)   // live favorite → flagged
        #expect(s2Fav == 0)   // tombstoned favorite → not flagged
        #expect(favCount == 1)
    }
