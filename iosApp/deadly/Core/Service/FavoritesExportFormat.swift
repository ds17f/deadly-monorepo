...
// MARK: - Legacy export envelope (v1/v2 import compatibility)

struct FavoritesExportLegacy: Codable {
    let version: Int
    let exportedAt: Int64
    let app: String
    let library: [FavoritesExportLegacyEntry]
    ...
    init(from decoder: Decoder) throws {
        ...
        library = try container.decode([FavoritesExportLegacyEntry].self, forKey: .library)
    }

    init(version: Int, exportedAt: Int64, app: String, library: [FavoritesExportLegacyEntry]) {
...
// MARK: - Per-show entry (v1/v2 legacy)

struct FavoritesExportLegacyEntry: Codable {
    let showId: String
...
// MARK: - Import result (legacy v1/v2)

struct FavoritesImportResult {
    let imported: Int
    let alreadyFavorited: Int
    let notFound: Int
}
