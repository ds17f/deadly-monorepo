...
object DatabaseMigrations {
     ...
     */
    /**
     * v24 → v25: relax favorite_songs natural key. Identity is now
     * (showId, trackTitle) — matches the server. recordingId stays as a
     * property on the row (used by the favorites screen for navigation)
     * but no longer participates in uniqueness.
     *
     * Dedupe: per (showId, trackTitle), keep the row with the highest
    ...
    val MIGRATION_24_25 = object : Migration(24, 25) {
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            """.trimIndent())
            db.execSQL("DELETE FROM favorite_songs WHERE id NOT IN (SELECT id FROM favorite_songs_keep)")
            db.execSQL("DROP TABLE favorite_songs_keep")

            // Step 2: swap the unique index. The old one was generated by
            // Room from the @Entity indices declaration; it lives as
            // `index_favorite_songs_showId_trackTitle_recordingId`.
            db.execSQL("DROP INDEX IF EXISTS index_favorite_songs_showId_trackTitle_recordingId")
            db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_favorite_songs_showId_trackTitle ON favorite_songs(showId, trackTitle)")
        }
    }
